/* ============================================
   HELPERS — Formatters, Calculators, Generators
   ============================================ */
...
export function formatDate(date, options = {}) {
  ...
  return new Intl.DateTimeFormat(CONFIG.LOCALE, defaultOptions).format(new Date(date));
}
...
export function generateStarRating(rating, maxStars = 5) {
  let html = '<div class="rating__stars" aria-label="Rating: ' + rating + ' out of ' + maxStars + '">';
  
  for (let i = 1; i <= maxStars; i++) {
    if (i <= Math.floor(rating)) {
      /* Full star */
      html += `<svg class="star--full" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
      </svg>`;
    } else if (i - 0.5 <= rating) {
      /* Half star */
      html += `<svg class="star--half" viewBox="0 0 20 20" aria-hidden="true">
        <defs><linearGradient id="halfGrad${i}"><stop offset="50%" stop-color="currentColor"/><stop offset="50%" stop-color="#cbd5e1"/></linearGradient></defs>
      ...
      /* Empty star */
      html += `<svg class="star--empty" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
      </svg>`;
    }
  }
...
/**
 * Generate product image placeholder with CSS gradient
 * @param {string} category 
 * @param {number} index 
 * @returns {string} CSS gradient
 */
export function generateProductGradient(category, index = 0) {
  const gradients = {
    fashion: [
  ...
  const categoryGradients = gradients[category?.toLowerCase()] || gradients.fashion;
 ...
 * @returns {string}
 */
export function getEstimatedDelivery(daysFromNow = 5) {
  const date = new Date();
...
/**
 * SVG icon map — inline SVGs for performance
 */
export const ICONS = {
  ...
  compare: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 3h5v5M4 20 21 3M21 16v5h-5M15 15l6 6M4 4l5 5"/></svg>',
